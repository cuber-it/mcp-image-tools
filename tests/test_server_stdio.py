...
mcp_image_tools.server.app` comes up as a process and answers on its pipes.
...
def ask(method: str, **params: object) -> dict[str, object]:
    ...
    finished = subprocess.run(
        [sys.executable, "-m", "mcp_image_tools.server.app", "--working-dir", "/tmp"],
...
def test_the_started_process_answers_on_its_pipes() -> None:
    ...
    assert len(answer["result"]["tools"]) == 5
